...
MODULE_PARM_DESC(buffSize, "Size of buffer");
...
int producer_thread_function(void *pv)
{
	...
	for_each_process(task)
	{
		if (task->cred->uid.val == uuid)
		{
			...
			buffer[fill].boot_time = ktime_get_ns();
...
int consumer_thread_function(void *pv)
{
	...
	while (!kthread_should_stop())
	{
		...
		unsigned long long start_time_ns = buffer[use].start_time;
        	unsigned long long current_time_ns = ktime_get_ns();
        	unsigned long long elapsed_time_ns = current_time_ns - start_time_ns;
        	total_time_elapsed += elapsed_time_ns;

        	no_of_process_consumed++;
        	total_no_of_process_consumed++;
        	PCINFO("[%s] Consumed Item#-%d on buffer index:%d::PID:%lu \n", current->comm, no_of_process_consumed, (use + buffSize - 1) % buffSize, buffer[use].pid);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Sherwin Wang");
MODULE_DESCRIPTION("CSE330 2023 Fall Project 2 Producer-Consumer Problem\n");
MODULE_VERSION("0.1");
